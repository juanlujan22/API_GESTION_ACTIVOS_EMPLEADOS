...
const createEmployee = async (values)=>{
   ...
   const  result= await conexion.query('INSERT INTO employees(first_name, last_name, cuit, team_id, join_date, rol) values(?,?,?,?,?,?)',
   [first_name, last_name, cuit, team_id, join_date, rol]).spread((result)=>result)// por la libreria mysql2-promise, el tema del callback
   return result
}
...
const updateEmployee = async (employee_id, values)=>{
    ...
    const  result= await conexion.query(sqlQuery,[first_name, last_name, cuit, team_id, join_date, rol]).spread((result)=>result)
    return result
}
